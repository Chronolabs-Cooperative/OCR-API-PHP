...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach($wizard->configs['gocr'] as $setting => $values)
    {
        $_SESSION['constants']['gocr'][$setting] = $_POST[$setting];
        if (is_file($_POST[$setting]))
        {
            $enabled[] = $setting;
        }
    }
    $_SESSION['constants']['apps']['enabled'] = implode(',', $enabled);
...
?>
    ...
    <div class="panel panel-info">
        ...
        <div class="form-group">
            <?php 
            foreach($wizard->configs['gocr'] as $setting => $default)
            {?>
